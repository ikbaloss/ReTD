...
class MyTabWidget(QTabWidget): 
    def __init__(self, parent): 
        ...
        #self.tabGraph = tabGraphInContext(parent = self) 
        #self.tabGraph.setVisible(False)
        #self.tabChord = tabChord(parent = self)
        #self.tabChord.setDisabled(True)
...
class tabConnection(QTabWidget):
    def __init__(self, parent): 
        ...
        #Save Graphml
        self.btGraphML = QPushButton()
        self.btGraphML.setText('GraphML')
        self.btGraphML.clicked.connect(self.createGraphML)
        #self.btGraphML.setEnabled(False)
        self.btGraphML.setVisible(False)
        #Save Plot Time
        ...
        self.glayout.addWidget(self.btGraphML, 1, 5)
    ...
    def selectPartition(self):
        ...
        if len(itemsUserHashtag) != 1:
            self.msgBox.setText("Select just one item")
        ...
        if not itemsUserHashtag[0] in self.partition.keys():
            self.msgBox.setText(itemsUserHashtag[0] + " is not in the graph")
            self.msgBox.setWindowTitle("ReTD")
            self.msgBox.setStandardButtons(QMessageBox.Ok)
            self.msgBox.show()
            return
    ...
    def createNetwork(self):
        ...
        self.btGraphML.setEnabled(True)
        def flatten(lst):
            flst = []
            for subl in lst:
                for e in subl:
        ...
        '''
        #The above green part is replaced by
        ...
        self.btDataTime.setVisible(False)
        self.btGraphML.setVisible(True)
        ...
        if pilihan == 'Tweet':
            ...
            if self.userCriteria != 'Followers': 
                ...
                if pilihan == 'User' and (self.userCriteria == 'RT' or self.userCriteria == 'Reply'):
                    ...
                    #print('Number total top ' + str(total_top))
